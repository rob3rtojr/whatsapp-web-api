...
const getColourScheme = require('docker/src/getColourScheme');
...
const createSession = function(id, description) {
  ...
  client.on('message',async function(res) {
    ...
    if (res.type.toLocaleLowerCase().trim()==="buttons_response") {
      
      console.log("Resposta......: " + await res.body);
      let quoted = await res.getQuotedMessage()
      console.log("quotedID.....: " + quoted.id.id);

      //resposta do agendamento
      //envia mensagem para o doutorweb:

      axios.post("http://doutorweb.azurewebsites.net/sistema/whatsapp/aoReceber.asp",{
        body: res.body,
...
// Send message
app.post('/send-message-button', async (req, res) => {
  ...
  let button = new Buttons(message, [{ body: 'Sim' }, { body: 'Não' }], 'Clínica Teste', 'mensagem enviada pelo sistema doutorweb');

  client.sendMessage(number, button).then(response => {
    console.log("id mensagem: " + response.id.id)
    res.status(200).json({
      status: true,
      response: response
    });
